 ...
 * for the 7-band semantics ([0]=air-dry rail ... [5]=submerged rail).
 *
 * CONFIDENCE: provisional. cal-source: 2026-06-28 common-cup / aggressive
 * wipe-&-air-dry characterization (Sage bench session; see
 * docs/experiments/2026-06-28-bench-session-log.md). Step 1 (#170) sets only the
 * per-channel OUTER rails [0]/[5] from that wet/dry envelope; the interior
 * [1..4] stays at the shared A2 values, so the WATERING decision is unchanged
 * until Step 2 (per-channel field-capacity anchor, gated on Sage's next round).
 * Caveat (Sage, carried verbatim — not sanded off): practical anchors with
 * margins, not absolute physics limits; s1/s4 dry endpoints still drift a little
 * with residual surface film / temperature, and s2 is wet-biased (gain+offset).
 */
