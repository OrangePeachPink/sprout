...
bool pump_pulse_active(const pump_pulse_t *p)    { return p->active; }
int  pump_pulse_channel(const pump_pulse_t *p)   { return p->active ? p->ch : -1; }
uint32_t pump_pulse_armed_ms(const pump_pulse_t *p) { return p->armed_ms; }
