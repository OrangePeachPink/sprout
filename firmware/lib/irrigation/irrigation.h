 ...
 *     merely because it took several doses. A slowly-wetting pot won't false-fault.
 *   - A structured logging seam (irrig_event_t) carrying everything a log record
 *     needs, so you can back it with Serial now and a file / WiFi sink later.
 *
 ...
 *   SYS_SETTLE   : all pumps OFF. Wait out motor/supply recovery, then resample.
 *
 * Watering is open-loop per dose (you cannot read a probe with its pump running)
 * and closed-loop across doses: dose -> soak lockout -> resample -> re-decide.
 *
 * Safety:
 *   - Only in-soil DRY *display* levels trigger watering. air-dry (probe not in
 *     soil), submerged, water-contact, or a tripped spread/health flag NEVER water.
 ...
 *   - No-improvement latch (dry tank / kinked tube / dead pump) stops a channel.
 ...
 * blocking delays, no analogRead/digitalWrite dependency, no dynamic allocation.
 *
 * --- LOGGING SEAM -----------------------------------------------------------
 * io.on_event(&irrig_event_t, user) fires at every decision/action. The event
 * carries {now_ms, ch, code, level, raw, spread} - everything a log record needs.
 * Wire it to Serial now for debugging; later back it with LittleFS/SD or push it
 * over WiFi/MQTT to a local server. Pass NULL to disable. Example stub:
 *
 ...
 * in [MOIST_DRY .. water_at_or_below]; consider the channel recovered once it is
 ...
 * sample (the engine does the burst + trimmed mean). set_pump handles the relay's
