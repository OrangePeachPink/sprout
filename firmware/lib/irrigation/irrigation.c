...
{
    switch (code) {
    case IRRIG_EV_LEVEL_CHANGE:
        return "level_change";
    case IRRIG_EV_PUMP_ON:
        return "pump_on";
    case IRRIG_EV_PUMP_OFF:
        return "pump_off";
    case IRRIG_EV_TARGET_REACHED:
        return "target_reached";
    case IRRIG_EV_PROBE_NOT_IN_SOIL:
        return "probe_not_in_soil";
    case IRRIG_EV_SENSOR_FAULT:
        return "sensor_fault";
    case IRRIG_EV_PUMP_OVERRUN_FAULT:
        return "pump_overrun_fault";
    case IRRIG_EV_NO_IMPROVEMENT_FAULT:
        return "no_improvement_fault";
    case IRRIG_EV_HEALTH_FAULT:
        return "health_fault";
    case IRRIG_EV_FAULT_CLEARED:
...
static int choose_channel(irrig_ctrl_t *c)
{
     ...
     * hard-faulted channel is never granted (clear it first). Rotate for
     * fairness. */
    ...
    for (int k = 0; k < IRRIG_CHANNELS; k++) {
        ...
        if (best < 0 || c->level[ch] < c->level[best])
            best = ch; /* lower idx = drier */
    }
    return best;
}
...
static void do_sweep(irrig_ctrl_t *c, uint32_t now)
{
    ...
    for (int ch = 0; ch < IRRIG_CHANNELS; ch++) {
        ...
        /* sensor-health (spread) veto - BACKLOG A1. A floating/disconnected
         * probe can report a plausible "dry" with a huge sample spread; never
         * act on it. The per-read veto suppresses watering immediately and
         * auto-recovers when the spread settles (a transient glitch). A
         * SUSTAINED warning - health_warn for max_health_warn consecutive
         * sweeps - escalates to a HARD latched fault (manual clear), because a
         * probe unhealthy that long is real hardware trouble a human should
         * look at. warn_count self-heals to 0 on any clean read, so only a
         * persistent fault reaches the threshold. max_health_warn
         * == 0 disables the latch; the per-read veto still applies. */
        if (c->mstate[ch].health_warn) {
            if (!c->prev_health_warn[ch])
                emit(c, ch, IRRIG_EV_SENSOR_FAULT, now);
        ...
        if (!moisture_level_is_display(lvl)) {
            if (lvl <= MOIST_AIR_DRY)
                emit(c, ch, IRRIG_EV_PROBE_NOT_IN_SOIL, now);
        ...
        /* soak expired and still dry: judge whether the last dose actually
         * helped */
        if (c->dosed_once[ch]) {
            int improvement =
                (int)c->raw_at_dose[ch] - (int)c->mstate[ch].last_raw;
                ...
                uint32_t now_ms)
{
    ...
    for (int ch = 0; ch < IRRIG_CHANNELS; ch++) {
        ...
        for (uint8_t d = 0; d < sys->adc_discard; d++)
            (void)io.read_raw(ch, io.user);
        for (uint16_t i = 0; i < mc->sample_count; i++)
            scratch[i] = io.read_raw(ch, io.user);
        uint16_t seed = moisture_trimmed_mean(scratch, mc->sample_count,
                                              mc->trim_each_side, NULL);
...
void irrig_tick(irrig_ctrl_t *c, uint32_t now)
{
    ...
    case SYS_SAMPLING: {
        ...
        if (ch >= 0) {
            /* Resolve the dose length: an operator forced dose (ADR-0016) uses
             * its requested ms (0 = the channel default), clamped to
             * pump_max_ms so the ceiling holds WITHOUT tripping the overrun
             * fault; autonomous uses the channel's dose_ms. Consume the
             * one-shot forced request here. */
            uint32_t dose = c->chan_cfg[ch].dose_ms;
            if (c->forced[ch]) {
                dose = c->forced_ms[ch] ? c->forced_ms[ch]
                                        : c->chan_cfg[ch].dose_ms;
            ...
            /* grant the single pump token; remember the pre-dose raw so the
             * next sweep can judge whether this dose actually wetted the soil
             */
            c->active_ch = ch;
            c->last_served = ch;
...
/* Operator forced dose (ADR-0016): queue a one-shot dose on `ch`. Granted on
 * the next SYS_SAMPLING tick (choose_channel gives it priority); the dose
 * length + ceiling clamp are resolved there. Refused for a bad channel or a
 * hard-faulted one. */
irrig_dose_result_t irrig_request_dose(irrig_ctrl_t *c, int ch, uint32_t ms)
{
    ...
    c->forced[ch] = true;
    c->forced_ms[ch] =
        ms; /* 0 = the channel's configured dose_ms; clamped at grant */
...
int irrig_active_pump(const irrig_ctrl_t *c)
{
    return c->active_ch;
}
