...
static inline pump_pulse_t *pump()
{
    return s_ctx.pump;
}
static inline unsigned long cadence()
{
    return *s_ctx.cadence_ms;
}
...
/* !cad,<ms>[,temp] - set the sweep cadence at runtime (ADR-0011 / #63).
 *   !cad,<ms>       persists to NVS — the operator's deliberate default.
 *   !cad,<ms>,temp  SESSION-ONLY: set live, NO NVS write; reverts to the saved/
 *                   compiled default on reset. Experiment Capture uses this so a
 *                   fast capture rate can't leak into the next monitor run (#322). */
...
static void handle_water(const char *args, char *reply, size_t replen)
{
    ...
    case PUMP_PULSE_ARMED:
        ...
        snprintf(reply, replen, "# ack water ch=%lu ms=%lu max=%lu",
                 (unsigned long)ch, (unsigned long)pump_pulse_armed_ms(pump()),
