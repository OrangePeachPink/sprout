...
| **2** | Contributor guide & domain glossary | A lean set: `AGENTS.md` (working rules incl. native-first + a challenger/architecture-review pass) + `CONTEXT.md` (domain glossary) + the ADRs | ✅ Maintainer / Firmware | ✅ `AGENTS.md` shipped; `CONTEXT.md` glossary still pending |
| **3** | Environment & dependencies | `uv`-managed Python (pinned interpreter + lockfile) + **PlatformIO/embedded toolchain pinning (`platformio.ini`)** + `.env.example`; secrets gitignored. Firmware owns the foundation; Data declares its analytics deps into it | ✅ Firmware (Data declares deps) | ✅ `uv`-managed (`pyproject.toml` + `uv.lock`, #114); `platformio.ini` pinned + `.env.example` |
| **4** | Task runner & rituals | A `justfile` exposing `start` / `check` / `ship`; `start` is the runner **plumbing** that launches the host application surface (Data-owned, ADR-0005) + the firmware/device side | ✅ Firmware | ✅ repo-root `justfile` — `start`/`check`/`serve`/`test`/`lint` (#97) |
...
| **6** | Spec & requirements | Specs/PRDs as `docs/prd/` markdown + an ideas inbox (Discussions); decomposed to issues — **see [ADR-0003](0003-work-pipeline.md)** | ✅ Workflow | ✅ pipeline live — PRDs in `docs/prd/`, Discussions inbox, issues + board; backlog doc retired |
| **7** | Backlog & issue tracking | **GitHub Issues (ledger) + Projects (board); IDs = issue `#N`** — **see [ADR-0003](0003-work-pipeline.md)** | ✅ Workflow | ✅ GitHub Issues + Project board #2; IDs are `#N`; backlog doc retired |
| **8** | Branching & merge | Short branch → PR → squash → manual review (auto-merge earned later); the flow is in [ADR-0003 §8](0003-work-pipeline.md); all lanes cut over at once when no-fly lifts | ✅ **policy: Workflow · mechanics: Firmware · ratify: Maintainer** | ✅ branch → PR → squash; a branch-protection ruleset requires a PR on `main` |
...
| **10** | Quality gates | `pre-commit` (fast hygiene/format/lint) + slower checks in CI — **harness owned here; each lane plugs in its own checks** | ✅ Firmware | ✅ `pre-commit` orchestration (#118) over `ruff` / `clang` / `markdownlint` / `cspell` |
...
| **12** | Continuous integration | GitHub Actions, hosted, path-filtered (green-or-not) — **harness owned here; lanes' checks plug in** | ✅ Firmware | 🟡 `ci.yml` (#103) — hosted lint+tests via `just check`; open PR, billing-parked to July 1 |
...
| **18** | Design system | Design tokens (color/type/space/radius) as CSS custom properties + a small component set — ratified in [ADR-0004](0004-design-system.md); brand in [ADR-0007](0007-brand-guidelines.md); v3 in [ADR-0008](0008-design-system-v3-personality-layer.md) | ✅ Design | 📋 **already built** — v1 + v2 under `docs/design/` |
