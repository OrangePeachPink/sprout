...
| [0033](0033-two-surface-architecture-home-and-workbench.md) | Two-surface architecture: **Home** (the designed user surface — mark, mood, first-person voice, single-plant hero, onboarding) + **Workbench** (today's Monitor/Capture/Lab/Diagnostics instrument, kept); one app, one data spine; the **honesty law applies to both** — mood is 1:1 from the calibrated band (never the index), character layers onto the instrument (the character↔instrument boundary made structural); Home is the front door + Workbench reachable from it; first-run → onboarding, returning → Home; **shell-first serving** (#1018 — the shell must not pay another surface's data cost); realizes #875 (extends ADR-0008 + ADR-0032) | **Proposed** — drafted by Trellis 2026-07-12 before the #1039 grill (frames Round 1); the grill amends + ratifies; V1 lands via the maintainer | Trellis (structure) + Design-QA (surfaces, #875) + Data (seams) |
