...
| [0014](0014-operator-control-plane.md) | Operator control plane (Monitor + Experiment under one plane; extends ADR-0011) | **Proposed** — Operator-Experience epic #125; §5 `serve.py` boundary added + Trellis-aligned (#296), ready for maintainer ratification | Data lane |
...
| [0018](0018-dual-mode-transport-and-durability.md) | Dual-mode transport & durability: source-adapter seam + device-owned time, one schema across transports (untethered; extends ADR-0006) | **Proposed** — drafted by Workflow from #243, Trellis-revised 2026-06-28; ratification gated on schema v2 (#300); awaiting maintainer + Data confirm (#268) | Data lane / architecture (cross-lane: Firmware) |
...
| [0021](0021-parse-v1-telemetry-contract-boundary.md) | parse_v1 is the single telemetry contract boundary (extends ADR-0006) | **Proposed** — drafted by Trellis; **Data-confirmed 2026-06-30** (#294/#295 landed; boundary verified — header bounds authoritative, `DEFAULT_CAL_BOUNDS` a labelled fallback, value=NULL honored); awaiting maintainer ratification | Trellis (author) + Data lane |
...
| [0023](0023-contextual-env-columns.md) | Contextual env columns: `*_context_*` is multi-source with a `context_source` provenance tag (one source per row, never blended), nearest-calibrated precedence, trust class travels with the value (extends ADR-0006) | **Proposed** — drafted by Data from #418; model ratifiable now, the `context_source` schema/parse_v1 column add needs Workflow/Trellis ratification | Data (author) + architecture (cross-lane: Firmware emit) |
| [0024](0024-multiplatform-pinning.md) | Multi-platform toolchain pinning: one *isolated, exact* pin per platform target (a new board = a new pinned env, never un-pinning the shipping matrix); whole-matrix moves are separately justified (extends ADR-0019 / #283) | **Proposed** — drafted by Trellis from #442 (DX-aligned); first instance esp32c5 is DX-wired; awaiting maintainer ratification | Trellis (author) + DX (wiring) |
