...
| [0029](0029-plant-pot-site-profile-registry.md) | Plant / pot / site profile registry: a slowly-changing **dimension** (pot geometry, hydrology, care history) keyed by the stable `plant_id`, joined to telemetry facts in the analysis tier — *not* identity, *not* telemetry; storage mirrors the device registry (committed schema + gitignored local instance, ADR-0015); every field absent-safe (ADR-0028); the covariate set the predictor conditions on (extends ADR-0027 / ADR-0006) | **Proposed** — drafted by Trellis 2026-07-06 from #675 (schema half); loader + analysis-tier join + seed back-fill are Data's half (v0.8.0, feeds PRD-0008) | Trellis (schema) + Data (loader) |
| [0030](0030-version-identity-and-display-contract.md) | Version identity, build provenance & display contract: name every versioned thing (product · fw semver · **build-instance id** = git-hash+timestamp · wire `schema_version` · `config_id` · server) with one owner-constant + one authoritative display each; the OTA receipt = a changed build-instance id on any push (same-source included); fw semver bumps **before** a coordinated reflash; masthead = product + live-fleet fw (retired excluded), Diagnostics = full table (elaborates ADR-0009) | **Proposed** — drafted by Trellis 2026-07-07 from #831 (bench-gap draft); scheme + packaging fork (new ADR vs ADR-0009 §7) to v0.8.0 planning | Trellis (scheme) + Firmware (build-id/OTA) + Design (display) + Workflow (bump ritual) |
| [0029](0029-plant-pot-site-profile-registry.md) | Plant / pot / site profile registry: a slowly-changing **dimension** (pot geometry, hydrology, care history) keyed by the stable `plant_id`, joined to telemetry facts in the analysis tier — *not* identity, *not* telemetry; storage mirrors the device registry (committed schema + gitignored local instance, ADR-0015); every field absent-safe (ADR-0028); the covariate set the predictor conditions on (extends ADR-0027 / ADR-0006) | **Proposed** — drafted by Trellis 2026-07-06 from #675 (schema half), revised 2026-07-07 to harden the three hard cases (nominal-vs-effective pot volume, drainage pathology, soil-condition) + probe-reading caveat + provenance (from #834 evidence); loader + analysis-tier join + seed back-fill are Data's half (v0.8.0, feeds PRD-0008) | Trellis (schema) + Data (loader) |
| [0031](0031-read-path-rollup-tiers.md) | Read-path rollup tiers: materialized aggregates over immutable raw — raw is Tier 0 (immutable, kept forever), Tier 1/2/3 are **derived, disposable, rebuilt-from-raw** rollups picked by window; the envelope contract (`mean/min/max/spread/n` + quality rollup per `(device_id, channel)`, over `raw_value`, per-board only); **events never downsampled** (band transitions, waterings, faults, sessions survive at exact timestamps in every tier); rollups labeled + rendered as envelopes, never fake-smooth (realizes/extends ADR-0006 §3) | **Proposed** — drafted by Trellis 2026-07-07 from #828 (bench-gap draft); substrate + granularity + config_id-boundary forks to v0.8.0 planning; Data builds, sequenced after #827 | Trellis (contract) + Data (materializer) |
