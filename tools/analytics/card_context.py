#!/usr/bin/env python3
"""The card-payload context — layer 3 (ADR-0038 §1, §5.3).

`build_context` composes every analysis the surfaces read: trajectory, bands, gaps,
fleet health, quality, provenance. It is the most depended-upon function in the host —
**36 test files and 138 call sites** — and until now it lived inside the module that
also owns the HTML template, the renderers, and the CLI entry point. Features kept
landing on the card-payload path, so the delivery module kept growing, and the boundary
between "what the data says" and "how it is served" existed only in people's heads.

**The defect this forecloses** (ADR-0038 §6 requires one to be named). #1315: the v5
channel rename folded correctly in the parse and tier paths and did *not* fold in the
live Home's registry join, and the Home lost all eight probed plants. The join sites
were scattered because the composition step had no module of its own — no single place
that owns "assemble what a surface is told". A path that is not a module cannot be
audited as one.

**This was a faithful move, not a rewrite.** The text was copied, not retyped; the
`build_context` golden-output suite (#1336, PR #1411) was written first, precisely so
the cut has something to be judged against. It pins the 19-key surface, every type, and
the values a fixed input produces. If that suite is green, the payload is unchanged.

**The seam is one-way and was measured before cutting:** the moved cluster references
nothing left behind in `dashboard`, and the two shared path constants moved to the
`host_paths` leaf first. `dashboard` now imports *downward* from here and re-exports
these names, so existing readers are unaffected while the layer rule holds.
"""
