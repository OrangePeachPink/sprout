...
static void t_health_veto_and_latch(void)
{
    ...
    CHECK(RIG.ev[IRRIG_EV_HEALTH_FAULT] == 1,
...
{
    printf("  two dry channels: single-pump + fairness + "
           "no-sample-while-pumping\n");
...
static void t_no_improvement_fault(void)
{
    ...
    CHECK(RIG.ev[IRRIG_EV_NO_IMPROVEMENT_FAULT] >= 1,
...
static void t_last_water_ms(void)
{
    ...
    CHECK(irrig_last_water_ms(&CTRL, 1) > at_init,
...
{
    printf("  band boundaries vs calibration anchors (issue #3; #248 endpoints "
           "ratified)\n");
    ...
    CHECK(band_of(3170) == MOIST_AIR_DRY,
          "#248 air-dry center 3170 -> air-dry");
    CHECK(band_of(3151) == MOIST_AIR_DRY,
          "#248 air-dry min (s2 3151) -> air-dry");
    CHECK(band_of(978) == MOIST_SUBMERGED,
          "#248 saturated center 978 -> submerged");
    CHECK(band_of(926) == MOIST_SUBMERGED,
          "#248 saturated min (s2 wet-bias 926) -> submerged");
    CHECK(band_of(1020) == MOIST_SUBMERGED,
...
static void t_serial_cmd_registry(void)
{
    ...
    CHECK(serial_cmd_dispatch("plants.soil,s3,...", reply, sizeof(reply)) ==
                  SERIAL_CMD_IGNORED &&
              reply[0] == '\0',
    ...
    CHECK(serial_cmd_dispatch("!cad,1000*00", reply, sizeof(reply)) ==
    ...
    CHECK(serial_cmd_dispatch(buf, reply, sizeof(reply)) ==
    ...
    CHECK(!serial_cmd_parse_u32("1234567890", &v),
...
static void t_pump_pulse(void)
{
    ...
    CHECK(!pump_pulse_active(&p) && pump_pulse_channel(&p) == -1,
          "init -> idle, no channel");

    /* arm with the default duration */
    CHECK(pump_pulse_arm(&p, 1, 0, 10000) == PUMP_PULSE_ARMED,
    ...
    CHECK(pump_pulse_arm(&p, 2, 500, 10100) == PUMP_PULSE_ERR_BUSY,
    ...
    CHECK(!pump_pulse_service(&p, 11000) && pump_pulse_active(&p),
          "service before expiry -> still on");
    CHECK(pump_pulse_service(&p, 11500),
          "service at expiry -> OFF signal (fires once)");
    CHECK(!pump_pulse_active(&p) && pump_pulse_channel(&p) == -1,
          "expired -> idle");
    CHECK(!pump_pulse_service(&p, 11600),
          "service after expiry -> no repeat signal");

    /* an over-ceiling request is clamped to max */
    CHECK(pump_pulse_arm(&p, 0, 99999, 20000) == PUMP_PULSE_ARMED,
          "arm over-ceiling -> ARMED");
    CHECK(pump_pulse_armed_ms(&p) == 5000,
    ...
    CHECK(pump_pulse_arm(&p, 4, 0, 30000) == PUMP_PULSE_ERR_CHANNEL,
          "arm ch4 (n=4) -> ERR_CHANNEL");
    CHECK(pump_pulse_arm(&p, -1, 0, 30000) == PUMP_PULSE_ERR_CHANNEL,
    ...
    CHECK(pump_pulse_arm(&p, 3, 1000, 0xFFFFFE00u) == PUMP_PULSE_ARMED,
          "arm near uint32 max -> ARMED");
    CHECK(!pump_pulse_service(&p, 0xFFFFFF00u) && pump_pulse_active(&p),
          "pre-expiry across wrap -> still on");
    CHECK(pump_pulse_service(&p, 0x00000300u),
...
static void t_forced_dose(void)
{
    ...
    CHECK(irrig_request_dose(&CTRL, 2, 0) == IRRIG_DOSE_QUEUED,
          "request ch2 -> queued");
    ...
    CHECK(RIG.pump_on[2] == false && RIG.pumps_on_now == 0,
    ...
    start();
    step(SYS.sample_period_ms);
    CHECK(irrig_request_dose(&CTRL, 0, 99999) == IRRIG_DOSE_QUEUED,
    ...
    start();
    CTRL.faulted[1] = true;
    CHECK(irrig_request_dose(&CTRL, 1, 0) == IRRIG_DOSE_FAULTED,
          "faulted channel -> refused");
    CHECK(CTRL.forced[1] == false, "refused request leaves no pending dose");

    /* (d) out-of-range channel rejected. */
    CHECK(irrig_request_dose(&CTRL, IRRIG_CHANNELS, 0) ==
              IRRIG_DOSE_BAD_CHANNEL,
          "ch=N -> bad channel");
    CHECK(irrig_request_dose(&CTRL, -1, 0) == IRRIG_DOSE_BAD_CHANNEL,
          "ch=-1 -> bad channel");
    ...
    CHECK(RIG.pumps_on_now == 1 && RIG.max_pumps_on == 1,
